...
RSpec.describe Puzzle2::Grid do
  def p(*values)
    values.map do |row, column|
      Puzzle2::Position.new(row, column)
    end
  end
  ...
  describe '.find_at' do
    ...
    with '.@.x.#.' do
      def self.at(row:, column:, &block)
        position = Puzzle2::Position.new(row, column)
  ...
  describe '.numbers_at' do
    let(:grid) { described_class.parse(definition) }
    subject { grid.numbers_at(*positions) }
    ...
    with '.1.#.' do
      ...
      at row: 0, column: 1 do
        it { is_expected.to contain_exactly(an_object_having_attributes(content: "1")) }
      end
      ...
      context "at [0,1], [0,2] and [0,3]" do
        let(:positions) { p([0,1],[0,2],[0,3]) }
        it { is_expected.to contain_exactly(an_object_having_attributes(content: "1")) }
      end
    end
  end
  ...
  describe '.stars' do
    ...
    # ..35..633.
    with '467..114&.|...*......|..35..633.' do
      it { is_expected.to match_array(an_object_having_attributes(content: "*", row: 1, column: 3)) }
    end
  end
  ...
  describe '.gears' do
    ...
    # ..35..633.
    with '467..114&.|...*......|..35..633.' do
      it { is_expected.to match_array(an_object_having_attributes(numbers: [467, 35])) }
    end
  end
end

RSpec.describe Puzzle2::Content do
  subject(:content) { Puzzle2::Content.new(content: 'dummy', row: 0, column: 0) }
